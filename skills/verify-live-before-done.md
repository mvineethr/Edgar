...
2. Exercise the changed path against the REAL upstream:
   - SEC paths: run the actual CLI (`howlet search berkshire`,
     `howlet holdings buffett`, `howlet diff buffett`).
   - Dashboard paths: start `howlet dashboard`, then check BOTH the JSON
     endpoint (`curl` the `/api/...` route) and the rendered screen in the
     browser preview.
   - MCP paths: `call_tool(...)` against a running `howlet mcp`, don't
...
live run ever (`howlet search berkshire` via Docker) found that SEC's
