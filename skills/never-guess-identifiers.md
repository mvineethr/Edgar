...
1. **Manager CIKs** (`presets.py`): resolve every candidate via
   `howlet search <name>` against live EDGAR before adding. EDGAR names
   ...
   `howlet holdings <cik>` once to confirm the portfolio looks like the
   manager you meant. CLAUDE.md's roadmap states this rule verbatim:
   "verify each CIK via `howlet search` before adding; never guess."
