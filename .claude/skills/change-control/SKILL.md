...
### Worked example (howlet, 2026-07-07)

Running this runbook on the exemplar repo (`D:\src\howlet`) yields, among
...
Step 4 finds the enforcement point in `src/howlet/client.py`: the
...
### Worked examples (howlet — all four verified against CLAUDE.md, HANDOVER.md, and the code, 2026-07-07)
...
SCOPE:     EdgarClient constructor (src/howlet/client.py); every SEC call.
...
- Written 2026-07-07 against the howlet exemplar (`D:\src\howlet`).
- All exemplar incidents verified against `HANDOVER.md` (sessions
  ...
  directly in `src/howlet/client.py`.
...
- Re-verify: `python -m pytest tests -q  # or your project's suite` for
  the count; `grep -n '"@" not in user_agent' src/howlet/client.py` and
  `grep -n '_MIN_INTERVAL_SECONDS' src/howlet/client.py` for the
